...
describe UsersController, :type => :controller do
  ...
  ***REMOVED***it 'activates user' do
  ***REMOVED***  User.authenticate('aaron', 'monkey').should be_nil
  ***REMOVED***  get :activate, :activation_code => users(:aaron).activation_code
  ***REMOVED***  response.should redirect_to('/login')
  ***REMOVED***  flash[:notice].should_not be_nil
  ***REMOVED***  flash[:error ].should     be_nil
  ***REMOVED***  User.authenticate('aaron', 'monkey').should == users(:aaron)
  ***REMOVED***end
  
  ***REMOVED***it 'does not activate user without key' do
  ***REMOVED***  get :activate
  ***REMOVED***  flash[:notice].should     be_nil
  ***REMOVED***  flash[:error ].should_not be_nil
  ***REMOVED***end
  
  ***REMOVED***it 'does not activate user with blank key' do
  ***REMOVED***  get :activate, :activation_code => ''
  ***REMOVED***  flash[:notice].should     be_nil
  ***REMOVED***  flash[:error ].should_not be_nil
  ***REMOVED***end
  
  ***REMOVED***it 'does not activate user with bogus key' do
  ***REMOVED***  get :activate, :activation_code => 'i_haxxor_joo'
  ***REMOVED***  flash[:notice].should     be_nil
  ***REMOVED***  flash[:error ].should_not be_nil
  ***REMOVED***end
...
describe UsersController do
  ...
  describe "named routing" do
	...
	***REMOVED*** The formatted_*_users_path is no longer in Rails 2.3+ so these tests won't work
    
    ***REMOVED*** it "should route users_path() to /users" do
      ***REMOVED*** users_path().should == "/users"
      ***REMOVED*** formatted_users_path(:format => 'xml').should == "/users.xml"
      ***REMOVED*** formatted_users_path(:format => 'json').should == "/users.json"
    ***REMOVED*** end
    
    ***REMOVED*** it "should route new_user_path() to /users/new" do
      ***REMOVED*** new_user_path().should == "/users/new"
      ***REMOVED*** formatted_new_user_path(:format => 'xml').should == "/users/new.xml"
      ***REMOVED*** formatted_new_user_path(:format => 'json').should == "/users/new.json"
    ***REMOVED*** end
    
    ***REMOVED*** it "should route user_(:id => '1') to /users/1" do
      ***REMOVED*** user_path(:id => '1').should == "/users/1"
      ***REMOVED*** formatted_user_path(:id => '1', :format => 'xml').should == "/users/1.xml"
      ***REMOVED*** formatted_user_path(:id => '1', :format => 'json').should == "/users/1.json"
    ***REMOVED*** end

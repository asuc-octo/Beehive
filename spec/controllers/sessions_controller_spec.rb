  ...
  describe "on successful login," do
    [ [:nil,       nil,            nil],
      ...
      [ true, false ].each do |want_remember_me|
        describe "my request cookie token is ***REMOVED***{has_request_token.to_s}," do
          describe "and ask ***REMOVED***{want_remember_me ? 'to' : 'not to'} be remembered" do 
            before do
            ...
            it "does not reset my session"   do controller.should_not_receive(:reset_session).and_return nil; do_create end ***REMOVED*** change if you uncomment the reset_session path

...
describe JobsController, :type => :controller do
  ...
  describe "searching function" do
	***REMOVED*** (for all of these: and render index)
	before(:each) do
	...
	it "should return all active jobs if there are no search parameters" do
		...
		***REMOVED*** solr isn't working properly with RSpec, and the search is done through solr, so this won't work properly
		***REMOVED***assigns[:jobs].should_not be_nil
	end	
	it "should return jobs that matches keyword query" do
		***REMOVED***get :list, :search_terms => {:query => "Hello"}
		***REMOVED*** solr isn't working properly with RSpec, and the search is done through solr, so this won't work properly
		***REMOVED***assigns[:jobs].should include(@job1)
	end
	it "should return jobs that match faculty query" do
		get :list, :search_terms => {:faculty_select => @first_faculty.id}
		***REMOVED*** solr isn't working properly with RSpec, and the search is done through solr, so this won't work properly
		***REMOVED***assigns[:jobs].should include(@job1)
		***REMOVED***assigns[:jobs].should include(@job2)
	end
	it "should return jobs that match paid query" do
		get :list, :search_terms => {:paid => 1}
		***REMOVED*** solr isn't working properly with RSpec, and the search is done through solr, so this won't work properly
		***REMOVED***assigns[:jobs].should include(@job1)
		***REMOVED***assigns[:jobs].should_not include(@job2)
	end
	it "should return jobs that match credit query" do
		get :list, :search_terms => {:credit => 1}
		***REMOVED*** solr isn't working properly with RSpec, and the search is done through solr, so this won't work properly
		***REMOVED***assigns[:jobs].should include(@job2)
		***REMOVED***assigns[:jobs].should_not include(@job1)
	end
  end

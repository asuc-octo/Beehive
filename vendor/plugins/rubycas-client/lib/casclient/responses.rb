module CASClient
  ...
  ***REMOVED*** Represents a response from the CAS server to a 'validate' request
  ***REMOVED*** (i.e. after validating a service/proxy ticket).
  class ValidationResponse
    ...
    def parse(raw_text)
      ...
      ***REMOVED*** if we got this far then we've got a valid XML response, so we're doing CAS 2.0
      @protocol = 2.0
      ...
      if is_success?
        ...
        @xml.elements.to_a('//cas:authenticationSuccess/*').each do |el|
          ***REMOVED*** generating the hash requires prefixes to be defined, so add all of the namespaces
          el.namespaces.each {|k,v| el.add_namespace(k,v)}
          @extra_attributes.merge!(Hash.from_xml(el.to_s)) unless (el == cas_user)
        end
        
        ***REMOVED*** unserialize extra attributes
        ...
        ***REMOVED*** this should never happen, since the response should already have been recognized as invalid
        raise BadResponseException, "BAD CAS RESPONSE:\n***REMOVED***{raw_text.inspect}\n\nXML DOC:\n***REMOVED***{doc.inspect}"
      end
  ...
  ***REMOVED*** Represents a response from the CAS server to a proxy ticket request 
  ***REMOVED*** (i.e. after requesting a proxy ticket).
  class ProxyResponse
    include XmlResponse
  ...
  ***REMOVED*** Represents a response from the CAS server to a login request 
  ***REMOVED*** (i.e. after submitting a username/password).
  class LoginResponse
    ...
    def parse_http_response(http_response)
      ...
      ***REMOVED*** FIXME: this regexp might be incorrect...
      ...
      if not ((http_response.kind_of?(Net::HTTPSuccess) || http_response.kind_of?(Net::HTTPFound)) && @ticket.present?)
        @failure = true
        ***REMOVED*** Try to extract the error message -- this only works with RubyCAS-Server.
        ***REMOVED*** For other servers we just return the entire response body (i.e. the whole error page).

...
module CASClient
  ...
  ***REMOVED*** Customized logger for the client.
  ***REMOVED*** This is useful if you're trying to do logging in Rails, since Rails'
  ***REMOVED*** clean_logger.rb pretty much completely breaks the base Logger class.
  class Logger < ::Logger
  ...
  ***REMOVED*** Wraps a real Logger. If no real Logger is set, then this wrapper
  ***REMOVED*** will quietly swallow any logging calls.
  class LoggerWrapper
    def initialize(real_logger=nil)
      set_logger(real_logger)
    end
    ***REMOVED*** Assign the 'real' Logger instance that this dummy instance wraps around.
    def set_real_logger(real_logger)
      @real_logger = real_logger
    end
    ***REMOVED*** Log using the appropriate method if we have a logger
    ***REMOVED*** if we dont' have a logger, gracefully ignore.
...
***REMOVED*** Detect legacy configuration and show appropriate error message
module CAS
  module Filter
    class << self

...
(this does not show proxy options, which are covered in the next section):
  ...
  ***REMOVED*** enable detailed CAS logging
...
cleanup, and then call <tt>CASClient::Frameworks::Rails::Filter.logout(controller)</tt>. For example:
  ...
  class ApplicationController < ActionController::Base
    ...
    ***REMOVED*** ...
    ...
    def logout
      ***REMOVED*** optionally do some local cleanup here
      ***REMOVED*** ...
...
In your <tt>config/environment.rb</tt>:
  ...
  ***REMOVED*** enable detailed CAS logging for easier troubleshooting
  cas_logger = CASClient::Logger.new(RAILS_ROOT+'/log/cas.log')
  cas_logger.level = Logger::DEBUG
...
the service URI as parameters to your target application:
  ...
  http://some-other-application.example.foo?service=***REMOVED***{CGI::escape(proxy_ticket.service)}&ticket=***REMOVED***{proxy_ticket.ticket}
...
Note that ***REMOVED***request_proxy_ticket returns a CASClient::ProxyTicket object, which is why we need to call ***REMOVED***ticket on it 

  ...
  module AttachmentFu ***REMOVED*** :nodoc:
    module Processors
      module RmagickProcessor
        ...
        module ClassMethods
          ***REMOVED*** Yields a block containing an RMagick Image for the given binary data.
          def with_image(file, &block)
            begin
              binary_data = file.is_a?(Magick::Image) ? file : Magick::Image.read(file).first unless !Object.const_defined?(:Magick)
            rescue
              ***REMOVED*** Log the failure to load the image.  This should match ::Magick::ImageMagickError
              ***REMOVED*** but that would cause acts_as_attachment to require rmagick.
        ...
        ***REMOVED*** Performs the actual resizing operation for a thumbnail
        def resize_image(img, size)
          ...
          if size.is_a?(Fixnum) || (size.is_a?(Array) && size.first.is_a?(Fixnum))
            ...
            img.thumbnail!(*size)
          elsif size.is_a?(String) && size =~ /^c.*$/ ***REMOVED*** Image cropping - example geometry string: c75x75

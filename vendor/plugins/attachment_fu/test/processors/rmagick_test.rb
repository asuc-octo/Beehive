  ...
  if Object.const_defined?(:Magick)
    def test_should_create_image_from_uploaded_file
      assert_created do
        ...
        ***REMOVED***assert_equal 1784, attachment.size
    ...
    def test_should_create_thumbnail
      ...
      assert_created do
        ...
        ***REMOVED***assert_in_delta 4673, thumbnail.size, 2
        ...
        assert_equal "***REMOVED***{basename}_thumb.***REMOVED***{ext}", thumbnail.filename
      end
    end
    ...
    def test_should_create_thumbnail_with_geometry_string
      ...
      assert_created do
        ...
        assert_valid thumbnail
        assert !thumbnail.size.zero?
        ***REMOVED***assert_equal 3915, thumbnail.size
    ...
    def test_should_resize_image(klass = ImageAttachment)
      ...
      assert  attachment.image?
      assert !attachment.size.zero?
      ***REMOVED***assert_in_delta 4673, attachment.size, 2
      assert_equal 50, attachment.width
    ...
    def test_should_resize_image_with_geometry(klass = ImageOrPdfAttachment)
      ...
      assert  attachment.image?
      assert !attachment.size.zero?
      ***REMOVED***assert_equal 3915, attachment.size
    ...
    def test_should_automatically_create_thumbnails(klass = ImageWithThumbsAttachment)
      ...
      assert_created 3 do
        ...
        ***REMOVED***assert_in_delta 4673, thumb.size, 2
        ...
        ***REMOVED***assert_equal 3915, geo.size
    ...
    ***REMOVED*** same as above method, but test it on a file model
    ...
    def test_should_use_thumbnail_subclass(klass = ImageWithThumbsClassFileAttachment)
      ...
      assert_equal attachment.thumbnails.first.full_filename, attachment.full_filename(attachment.thumbnails.first.thumbnail),
        "***REMOVED***full_filename does not use thumbnail class' path."
    ...
    def test_should_remove_old_thumbnail_files_when_updating(klass = ImageWithThumbsFileAttachment)
      ...
      assert_not_created do
        use_temp_file "files/rails.png" do |file|
          ...
          new_filenames.each { |f| assert  File.exists?(f), "***REMOVED***{f} does not exist" }
          old_filenames.each { |f| assert !File.exists?(f), "***REMOVED***{f} still exists" }
        end
      end
    end
    ...
    def test_should_delete_file_when_in_file_system_when_attachment_record_destroyed(klass = ImageWithThumbsFileAttachment)
      ...
      filenames.each { |f| assert  File.exists?(f),  "***REMOVED***{f} never existed to delete on destroy" }
      attachment.destroy
      filenames.each { |f| assert !File.exists?(f),  "***REMOVED***{f} still exists" }
    end
    ...
    def test_should_overwrite_old_thumbnail_records_when_updating(klass = ImageWithThumbsAttachment)
      ...
      assert_not_created do ***REMOVED*** no new db_file records
        use_temp_file "files/rails.png" do |file|
          attachment.filename             = 'rails2.png'
          ***REMOVED*** The above test (***REMOVED***test_should_have_full_filename_method) to pass before be can set the temp_path below -- 
          ***REMOVED*** ***REMOVED***temp_path calls ***REMOVED***full_filename, which is not getting mixed into the attachment.  Maybe we don't need to
          ***REMOVED*** set temp_path at all?
          ***REMOVED***
          ***REMOVED*** attachment.temp_paths.unshift File.join(fixture_path, file)
          attachment.save!
        end
      end
    end

...
class S3Test < Test::Unit::TestCase
  ...
  if test_S3? && File.exist?(File.join(File.dirname(__FILE__), '../../amazon_s3.yml'))
    ...
    def test_should_create_valid_url(klass = S3Attachment)
      attachment_model klass
      attachment = upload_file :filename => '/files/rails.png'
      assert_equal "***REMOVED***{s3_protocol}***REMOVED***{s3_hostname}***REMOVED***{s3_port_string}/***REMOVED***{attachment.bucket_name}/***REMOVED***{attachment.full_filename}", attachment.s3_url
    end
    ...
    def test_should_create_authenticated_url_for_thumbnail(klass = S3Attachment)
      ...
      ['large', :large].each do |thumbnail|
        assert_match(
          ...
          "authenticated_s3_url failed with ***REMOVED***{thumbnail.class} parameter"
        )
      end
    end

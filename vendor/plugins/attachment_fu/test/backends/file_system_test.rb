...
class FileSystemTest < Test::Unit::TestCase
  ...
  def test_should_store_file_attachment_in_filesystem(klass = FileAttachment)
    ...
    assert_created do
      ...
      assert File.exists?(attachment.full_filename), "***REMOVED***{attachment.full_filename} does not exist"    
    end
    attachment
  end
  ...
  def test_should_delete_old_file_when_updating(klass = FileAttachment)
    ...
    assert_not_created do
      use_temp_file 'files/rails.png' do |file|
        ...
        attachment.save!
        assert  File.exists?(attachment.full_filename), "***REMOVED***{attachment.full_filename} does not exist"    
        assert !File.exists?(old_filename),             "***REMOVED***{old_filename} still exists"
      end
    end
  end
  ...
  def test_path_partitioning_works_on_integer_id(klass = FileAttachment)
    ...
    ***REMOVED*** Create a random attachment object, doesn't matter what.
    attachment = upload_file :filename => '/files/rails.png'

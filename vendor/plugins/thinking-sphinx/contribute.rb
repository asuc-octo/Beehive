...
EO_CREATE_DATABASE_FAILED
  ...
  ***REMOVED*** create database.yml
  def create_database_yaml
    ...
    colour_print " * <b>***REMOVED***{db_yml}</b>... "
    ...
    config.each do |(k,v)|
      colour_puts " * <b>***REMOVED***{k}</b>: ***REMOVED***{v}"
    end
...
module ContributeHelper
  ...
  class Gem < Dependency
    ...
    def name; "***REMOVED***{super} gem" end
  ...
  def check_for_dependencies
    ...
    dependencies.each do |klass|
      ...
      if dep.found?
        if dep.location
          colour_puts "<green>found at ***REMOVED***{dep.location}</green>"
        else
  ...
  def subs_colour(data)
    data = data.gsub(%r{<b>(.*?)</b>}m, colourise_output? ? "\e[1m\\1***REMOVED***{DEFAULT_TERMINAL_COLORS}" : MONOCHROME_OUTPUT)
    data.gsub!(%r{<red>(.*?)</red>}m, colourise_output? ? "\e[1m\e[31m\\1***REMOVED***{DEFAULT_TERMINAL_COLORS}" : MONOCHROME_OUTPUT)
    data.gsub!(%r{<green>(.*?)</green>}m, colourise_output? ? "\e[1m\e[32m\\1***REMOVED***{DEFAULT_TERMINAL_COLORS}" : MONOCHROME_OUTPUT)
    data.gsub!(%r{<yellow>(.*?)</yellow>}m, colourise_output? ? "\e[1m\e[33m\\1***REMOVED***{DEFAULT_TERMINAL_COLORS}" : MONOCHROME_OUTPUT)
    data.gsub!(%r{<banner>(.*?)</banner>}m, colourise_output? ? "\e[33m\e[44m\e[1m\\1***REMOVED***{DEFAULT_TERMINAL_COLORS}" : MONOCHROME_OUTPUT)

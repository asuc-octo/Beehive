...
describe ThinkingSphinx::Attribute do
  ...
  describe '***REMOVED***initialize' do
  ...
  describe '***REMOVED***unique_name' do
    before :each do
  ...
  describe '***REMOVED***to_select_sql' do
  ...
  describe '***REMOVED***all_datetimes?' do
  ...
  describe '***REMOVED***all_strings?' do
  ...
  describe "MVA with source query" do
    ...
    it "should use a query" do
      ...
      query.should       == "SELECT `tags`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `tags`.`id` AS `tag_ids` FROM `tags`"
    end
  end
  ...
  describe "MVA with source query for a delta source" do
    ...
    it "should use a query" do
      ...
      declaration.should == "uint tag_ids from query"
      query.should       == "SELECT `tags`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `tags`.`id` AS `tag_ids` FROM `tags` WHERE `tags`.`person_id` IN (SELECT `id` FROM `people` WHERE `people`.`delta` = 1)"
    end
  end
  ...
  describe "MVA via a HABTM association with a source query" do
    ...
    it "should use a ranged query" do
      ...
      query.should       == "SELECT `links_people`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `links_people`.`link_id` AS `link_ids` FROM `links_people`"
    end
  end
  ...
  describe "MVA with ranged source query" do
    ...
    it "should use a ranged query" do
      ...
      query.should       == "SELECT `tags`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `tags`.`id` AS `tag_ids` FROM `tags` WHERE `tags`.`person_id` >= $start AND `tags`.`person_id` <= $end"
  ...
  describe "MVA with ranged source query for a delta source" do
    ...
    it "should use a ranged query" do
      ...
      declaration.should == "uint tag_ids from ranged-query"
      query.should       == "SELECT `tags`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `tags`.`id` AS `tag_ids` FROM `tags` WHERE `tags`.`person_id` >= $start AND `tags`.`person_id` <= $end AND `tags`.`person_id` IN (SELECT `id` FROM `people` WHERE `people`.`delta` = 1)"
      range_query.should == "SELECT MIN(`tags`.`person_id`), MAX(`tags`.`person_id`) FROM `tags`"
    end
  end
  ...
  describe "MVA via a has-many :through with a ranged source query" do
    ...
    it "should use a ranged query" do
      ...
      query.should       == "SELECT `tags`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `tags`.`football_team_id` AS `football_team_ids` FROM `tags` WHERE `tags`.`person_id` >= $start AND `tags`.`person_id` <= $end"
      range_query.should == "SELECT MIN(`tags`.`person_id`), MAX(`tags`.`person_id`) FROM `tags`"
    end
  end
  ...
  describe "MVA via a has-many :through using a foreign key with a ranged source query" do
    ...
    it "should use a ranged query" do
      ...
      query.should       == "SELECT `friendships`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `friendships`.`friend_id` AS `friend_ids` FROM `friendships` WHERE `friendships`.`person_id` >= $start AND `friendships`.`person_id` <= $end"
  ...
  describe "MVA via a HABTM with a ranged source query" do
    ...
    it "should use a ranged query" do
      ...
      query.should       == "SELECT `links_people`.`person_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `links_people`.`link_id` AS `link_ids` FROM `links_people` WHERE `links_people`.`person_id` >= $start AND `links_people`.`person_id` <= $end"
  ...
  describe "MVA via two has-many associations with a ranged source query" do
    ...
    it "should use a ranged query" do
      ...
      query.should       == "SELECT `betas`.`alpha_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `gammas`.`value` AS `gamma_values` FROM `betas` LEFT OUTER JOIN `gammas` ON gammas.beta_id = betas.id WHERE `betas`.`alpha_id` >= $start AND `betas`.`alpha_id` <= $end"
  ...
  describe "MVA via two has-many associations with a ranged source query for a delta source" do
    ...
    it "should use a ranged query" do
      ...
      declaration, query, range_query = @attribute.config_value(nil, true).split('; ')
      declaration.should == "uint gamma_values from ranged-query"
      query.should       == "SELECT `betas`.`alpha_id` ***REMOVED***{ThinkingSphinx.unique_id_expression} AS `id`, `gammas`.`value` AS `gamma_values` FROM `betas` LEFT OUTER JOIN `gammas` ON gammas.beta_id = betas.id WHERE `betas`.`alpha_id` >= $start AND `betas`.`alpha_id` <= $end AND `betas`.`alpha_id` IN (SELECT `id` FROM `alphas` WHERE `alphas`.`delta` = 1)"
      range_query.should == "SELECT MIN(`betas`.`alpha_id`), MAX(`betas`.`alpha_id`) FROM `betas`"
    end
  end
  ...
  describe '***REMOVED***live_value' do
    before :each do
      @attribute = ThinkingSphinx::Attribute.new @source, [
        stub('column', :__stack => [], :__name => "col_name")
      ]

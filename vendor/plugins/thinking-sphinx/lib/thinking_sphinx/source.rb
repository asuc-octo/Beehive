  ...
  class Source
    ...
    def initialize(index, options = {})
      ...
      unless @model.descends_from_active_record?
        ...
        @conditions << "***REMOVED***{@model.quoted_table_name}.***REMOVED***{quote_column(@model.inheritance_column)} = '***REMOVED***{stored_class}'"
      end
    ...
    def to_riddle_for_core(offset, position)
      source = Riddle::Configuration::SQLSource.new(
        "***REMOVED***{index.core_name}_***REMOVED***{position}", adapter.sphinx_identifier
      )
    ...
    def to_riddle_for_delta(offset, position)
      source = Riddle::Configuration::SQLSource.new(
        "***REMOVED***{index.delta_name}_***REMOVED***{position}", adapter.sphinx_identifier
      )
      source.parent = "***REMOVED***{index.core_name}_***REMOVED***{position}"
    ...
    ***REMOVED*** Gets the association stack for a specific key.
    ***REMOVED*** 
    def association(key)
    ...
    def set_source_database_settings(source)
      ...
      source.sql_pass = (config[:password].to_s || "").gsub('***REMOVED***', '\***REMOVED***')
    ...
    def set_source_sql(source, offset, delta = false)
      ...
      if @index.local_options[:group_concat_max_len]
        source.sql_query_pre << "SET SESSION group_concat_max_len = ***REMOVED***{@index.local_options[:group_concat_max_len]}"
      end
    ...
    def set_source_settings(source)
      ...
      config.source_options.each do |key, value|
        source.send("***REMOVED***{key}=".to_sym, value)
      end
    ...
    ***REMOVED*** Returns all associations used amongst all the fields and attributes.
    ***REMOVED*** This includes all associations between the model and what the actual
    ***REMOVED*** columns are from.
    ***REMOVED*** 
    def all_associations
      @all_associations ||= (
        ***REMOVED*** field associations
        ...
        ***REMOVED*** explicit joins
        @joins.collect { |join|
          join.associations
        }.flatten
      ).uniq.collect { |assoc|
        ***REMOVED*** get ancestors as well as column-level associations
        assoc.ancestors
      }.flatten.uniq
    end

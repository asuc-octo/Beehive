...
DESC
      ...
      task :sphinx do
        ...
        begin
          ...
          puts "Continuing despite error: ***REMOVED***{e.message}"
        end
      
        args = []
        if with_postgres
          run "pg_config --pkgincludedir" do |channel, stream, data|
            args << "--with-pgsql=***REMOVED***{data}"
          end
        end
        ...
        cd sphinx-0.9.8.1
        ./configure ***REMOVED***{args.join(" ")}
        make
        ***REMOVED***{try_sudo} make install
      ...
      task :ts do
        run "***REMOVED***{try_sudo} gem install thinking-sphinx --source http://gemcutter.org"
      end
    end
    ...
    task :shared_sphinx_folder, :roles => :web do
      run "mkdir -p ***REMOVED***{shared_path}/db/sphinx/production"
    end
    ...
    def rake(*tasks)
      ...
      tasks.each do |t|
        run "if [ -d ***REMOVED***{release_path} ]; then cd ***REMOVED***{release_path}; else cd ***REMOVED***{current_path}; fi; ***REMOVED***{rake} RAILS_ENV=***REMOVED***{rails_env} ***REMOVED***{t}"
      end
    end
  end
end

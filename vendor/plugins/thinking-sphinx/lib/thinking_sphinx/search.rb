...
module ThinkingSphinx
  ***REMOVED*** Once you've got those indexes in and built, this is the stuff that
  ***REMOVED*** matters - how to search! This class provides a generic search
  ***REMOVED*** interface - which you can use to search all your indexed models at once.
  ***REMOVED*** Most times, you will just want a specific model's results - to search and
  ***REMOVED*** search_for_ids methods will do the job in exactly the same manner when
  ***REMOVED*** called from a model.
  ***REMOVED*** 
  class Search
    ...
    ***REMOVED*** Deprecated. Use ThinkingSphinx.search
    def self.search(*args)
      log 'ThinkingSphinx::Search.search is deprecated. Please use ThinkingSphinx.search instead.'
      ThinkingSphinx.search *args
    end
    
    ***REMOVED*** Deprecated. Use ThinkingSphinx.search_for_ids
    ...
    ***REMOVED*** Indication of whether the request has been made to Sphinx for the search
    ***REMOVED*** query.
    ***REMOVED*** 
    ***REMOVED*** @return [Boolean] true if the results have been requested.
    ***REMOVED*** 
    def populated?
      !!@populated
    end
    
    ***REMOVED*** The query result hash from Riddle.
    ***REMOVED*** 
    ***REMOVED*** @return [Hash] Raw Sphinx results
    ***REMOVED*** 
    def results
      populate
      @results
    ...
    ***REMOVED*** Returns true if the Search object or the underlying Array object respond
    ***REMOVED*** to the requested method.
    ***REMOVED*** 
    ***REMOVED*** @param [Symbol] method The method name
    ***REMOVED*** @return [Boolean] true if either Search or Array responds to the method.
    ***REMOVED*** 
    def respond_to?(method, include_private = false)
      super || @array.respond_to?(method, include_private)
    end
    
    ***REMOVED*** The current page number of the result set. Defaults to 1 if no page was
    ...
    ***REMOVED*** The next page number of the result set. If there are no more pages
    ***REMOVED*** available, nil is returned.
    ***REMOVED*** 
    ***REMOVED*** @return [Integer, nil]
    ***REMOVED*** 
    def next_page
      current_page >= total_pages ? nil : current_page + 1
    end
    
    ***REMOVED*** The previous page number of the result set. If this is the first page,
    ***REMOVED*** then nil is returned.
    ***REMOVED*** 
    ***REMOVED*** @return [Integer, nil]
    ***REMOVED*** 
    def previous_page
    ...
    ***REMOVED*** The amount of records per set of paged results. Defaults to 20 unless a
    ***REMOVED*** specific page size is requested.
    ***REMOVED*** 
    ***REMOVED*** @return [Integer]
    ***REMOVED*** 
    def per_page
    ...
    ***REMOVED*** The total number of pages available if the results are paginated.
    ***REMOVED*** 
    ***REMOVED*** @return [Integer]
    ***REMOVED*** 
    def total_pages
    ...
    ***REMOVED*** Compatibility with older versions of will_paginate
    alias_method :page_count, :total_pages
    
    ***REMOVED*** Query time taken
    ***REMOVED*** 
    ***REMOVED*** @return [Integer]
    ***REMOVED***
    def query_time
    ...
    ***REMOVED*** The total number of search results available.
    ***REMOVED*** 
    ***REMOVED*** @return [Integer]
    ***REMOVED*** 
    def total_entries
    ...
    ***REMOVED*** The current page's offset, based on the number of records per page.
    ***REMOVED*** Or explicit :offset if given. 
    ***REMOVED*** 
    ***REMOVED*** @return [Integer]
    ***REMOVED*** 
    def offset
      ...
      client.excerpts(
        {
          ...
          :index  => options[:index] || "***REMOVED***{model.source_of_sphinx_index.sphinx_name}_core"
    ...
    def populate
      ...
      retry_on_stale_index do
        begin
          log "Querying: '***REMOVED***{query}'"
          runtime = Benchmark.realtime {
            @results = client.query query, indexes, comment
          }
          log "Found ***REMOVED***{@results[:total_found]} results", :debug,
            "Sphinx (***REMOVED***{sprintf("%f", runtime)}s)"
    ...
    def self.log(message, method = :debug, identifier = 'Sphinx')
      ...
      if ::ActiveRecord::Base.colorize_logging
        identifier_color, message_color = "4;32;1", "0" ***REMOVED*** 0;1 = Bold
        info << "  \e[***REMOVED***{identifier_color}m***REMOVED***{identifier}\e[0m   "
        info << "\e[***REMOVED***{message_color}m***REMOVED***{message}\e[0m"
      else
        info = "***REMOVED***{identifier}   ***REMOVED***{message}"
      end
    ...
    def prepare(client)
      ...
      [
        ...
        client.send("***REMOVED***{key}=", value) if value
      end

      ***REMOVED*** treated non-standard as :select is already used for AR queries
    ...
    def retry_on_stale_index(&block)
      ...
      begin
        ...
        ***REMOVED*** If ThinkingSphinx::Search***REMOVED***instances_from_matches found records in
        ***REMOVED*** Sphinx but not in the DB and the :raise_on_stale option is set, this
        ***REMOVED*** exception is raised. We retry a limited number of times, excluding the
        ***REMOVED*** stale ids from the search.
      rescue StaleIdsException => err
        retries -= 1
        
        ***REMOVED*** For logging
        stale_ids |= err.ids
        ***REMOVED*** ID exclusion
    ...
    def conditions_as_query
      ...
      ***REMOVED*** Soon to be deprecated.
      keys = @options[:conditions].keys.reject { |key|
        attributes.include?(key.to_sym)
      }
      
      ' ' + keys.collect { |key|
        "@***REMOVED***{key} ***REMOVED***{options[:conditions][key]}"
      }.join(' ')
    end
    ...
    def star_query(query)
      ...
      query.gsub(/("***REMOVED***{token}(.*?***REMOVED***{token})?"|(?![!-])***REMOVED***{token})/u) do
        pre, proper, post = $`, $&, $'
        ***REMOVED*** E.g. "@foo", "/2", "~3", but not as part of a token
        is_operator = pre.match(%r{(\W|^)[@~/]\Z})
        ***REMOVED*** E.g. "foo bar", with quotes
    ...
    def sorted_fields_to_attributes(order_string)
      field_names.each { |field|
        order_string.gsub!(/(^|\s)***REMOVED***{field}(,?\s|$)/) { |match|
    ...
    ***REMOVED*** Turn :index_weights => { "foo" => 2, User => 1 } into :index_weights =>
    ***REMOVED*** { "foo" => 2, "user_core" => 1, "user_delta" => 1 }
    ***REMOVED*** 
    def index_weights
      ...
      weights.keys.inject({}) do |hash, key|
        if key.is_a?(Class)
          ...
          hash["***REMOVED***{name}_core"]  = weights[key]
          hash["***REMOVED***{name}_delta"] = weights[key]
        else
...
option, not :conditions. For example:
  :with => {:***REMOVED***{attrib} => ***REMOVED***{value.inspect}}
MSG
    ...
    ***REMOVED*** When passed a Time instance, returns the integer timestamp.
    ...
    def instances_from_class(klass, matches)
      ...
      ***REMOVED*** Raise an exception if we find records in Sphinx but not in the DB, so
      ***REMOVED*** the search method can retry without them. See 
      ***REMOVED*** ThinkingSphinx::Search.retry_search_on_stale_index.
      if options[:raise_on_stale] && instances.length < ids.length
        stale_ids = ids - instances.map { |i| i.id }
        raise StaleIdsException, stale_ids
      end

      ***REMOVED*** if the user has specified an SQL order, return the collection
      ***REMOVED*** without rearranging it into the Sphinx order
    ...
    ***REMOVED*** Group results by class and call ***REMOVED***find(:all) once for each group to reduce
    ***REMOVED*** the number of ***REMOVED***find's in multi-model searches.
    ***REMOVED*** 
    ...
    def each_with_attribute(attribute, &block)
      ...
      results[:matches].each_with_index do |match, index|
        yield self[index],
          (match[:attributes][attribute] || match[:attributes]["@***REMOVED***{attribute}"])
      end
    end
    ...
    ***REMOVED*** Adds the default_sphinx_scope if set.
    ...
    def add_scope(method, *args, &block)
      method = "***REMOVED***{method}_without_default".to_sym

...
module ThinkingSphinx
  class Index
    ...
    ***REMOVED*** Create a new index instance by passing in the model it is tied to, and
    ***REMOVED*** a block to build it with (optional but recommended). For documentation
    ***REMOVED*** on the syntax for inside the block, the Builder class is what you want.
    ***REMOVED***
    ***REMOVED*** Quick Example:
    ***REMOVED***
    ***REMOVED***   Index.new(User) do
    ***REMOVED***     indexes login, email
    ***REMOVED***     
    ...
    ***REMOVED***
    ...
    def set_configuration_options_for_indexes(index)
      config.index_options.each do |key, value|
        method = "***REMOVED***{key}=".to_sym
        index.send(method, value) if index.respond_to?(method)
      end
      
      options.each do |key, value|
        index.send("***REMOVED***{key}=".to_sym, value) if ThinkingSphinx::Configuration::IndexOptions.include?(key.to_s) && !value.nil?
      end
    end

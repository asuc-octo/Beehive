...
namespace :thinking_sphinx do
  ...
  task :start => :app_env do
    ...
    Dir["***REMOVED***{config.searchd_file_path}/*.spl"].each { |file| File.delete(file) }
    
    config.controller.start
    
    if sphinx_running?
      puts "Started successfully (pid ***REMOVED***{sphinx_pid})."
    else
      puts "Failed to start searchd daemon. Check ***REMOVED***{config.searchd_log_file}"
    end
  end
  ...
  task :stop => :app_env do
    unless sphinx_running?
      ...
      puts "Stopped search daemon (pid ***REMOVED***{pid})."
    end
  end
  ...
  task :configure => :app_env do
    config = ThinkingSphinx::Configuration.instance
    puts "Generating Configuration to ***REMOVED***{config.config_file}"
    config.build
  end

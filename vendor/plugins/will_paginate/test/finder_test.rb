...
class FinderTest < ActiveRecordTestCase
  ...
  def test_parameter_api
    ***REMOVED*** :page parameter in options is required!
    assert_raise(ArgumentError){ Topic.paginate }
    assert_raise(ArgumentError){ Topic.paginate({}) }
    
    ***REMOVED*** explicit :all should not break anything
    assert_equal Topic.paginate(:page => nil), Topic.paginate(:all, :page => 1)

    ***REMOVED*** :count could be nil and we should still not cry
  ...
  def test_paginate_with_per_page
    ...
    ***REMOVED*** Developer class has explicit per_page at 10
  ...
  def test_paginate_associations_with_include
    ...
    expected = Topic.find :all, 
      :include => 'replies', 
      :conditions => "project_id = ***REMOVED***{project.id} AND replies.content LIKE 'Nice%' ", 
      :limit   => 10
  ...
  def test_paginate_associations
    ...
    ***REMOVED*** has_many with implicit order
    topic = Topic.find(1)
  ...
  def test_paginate_with_dynamic_finder
    ...
    ***REMOVED*** dynamic finder + conditions
  ...
  ***REMOVED******REMOVED*** named_scope ***REMOVED******REMOVED***
  ...
  ***REMOVED******REMOVED*** misc ***REMOVED******REMOVED***
  ...
  ***REMOVED*** this functionality is temporarily removed
  def xtest_pagination_defines_method
    pager = "paginate_by_created_at"
    assert !User.methods.include_method?(pager), "User methods should not include `***REMOVED***{pager}` method"
    ***REMOVED*** paginate!
    assert 0, User.send(pager, nil, :page => 1).total_entries
    ***REMOVED*** the paging finder should now be defined
    assert User.methods.include_method?(pager), "`***REMOVED***{pager}` method should be defined on User"
  end

  ***REMOVED*** Is this Rails 2.0? Find out by testing find_all which was removed in [6998]
  unless ActiveRecord::Base.respond_to? :find_all
    def test_paginate_array_of_ids
      ***REMOVED*** AR finders also accept arrays of IDs
      ***REMOVED*** (this was broken in Rails before [6912])
      assert_queries(1) do
  ...
  uses_mocha 'internals' do
    ...
    def test_should_use_scoped_finders_if_present
      ***REMOVED*** scope-out compatibility
    ...
    ***REMOVED*** TODO: counts are still wrong
    def test_ability_to_use_with_custom_finders
      ***REMOVED*** acts_as_taggable defines find_tagged_with(tag, options)
    ...
    ***REMOVED*** detect ActiveRecord 2.1

...
class ArrayPaginationTest < Test::Unit::TestCase
  ...
  def test_guessing_total_count
    entries = create do |pager|
      ***REMOVED*** collection is shorter than limit
      pager.replace array
    end
    assert_equal 8, entries.total_entries
    
    entries = create(2, 5, 10) do |pager|
      ***REMOVED*** collection is shorter than limit, but we have an explicit count
      pager.replace array
    end
    assert_equal 10, entries.total_entries
    
    entries = create do |pager|
      ***REMOVED*** collection is the same as limit; we can't guess
      pager.replace array(5)
    end
    assert_equal nil, entries.total_entries
    
    entries = create do |pager|
      ***REMOVED*** collection is empty; we can't guess
      pager.replace array(0)
    end
    assert_equal nil, entries.total_entries
    
    entries = create(1) do |pager|
      ***REMOVED*** collection is empty and we're on page 1,
      ***REMOVED*** so the whole thing must be empty, too
      pager.replace array(0)
    end

...
class AuthenticatedGenerator < Rails::Generator::NamedBase
  ...
  attr_reader   :controller_name,
                ...
                :controller_routing_name,                 ***REMOVED*** new_session_path
                :controller_routing_path,                 ***REMOVED*** /session/new
                :controller_controller_name,              ***REMOVED*** sessions
  ...
  attr_reader   :model_controller_name,
                ...
                :model_controller_routing_name,           ***REMOVED*** new_user_path
                :model_controller_routing_path,           ***REMOVED*** /users/new
                :model_controller_controller_name         ***REMOVED*** users
  ...
  def initialize(runtime_args, runtime_options = {})
    ...
    ***REMOVED*** sessions controller
    ...
    if @controller_class_nesting.empty?
      ...
      @controller_class_name = "***REMOVED***{@controller_class_nesting}::***REMOVED***{@controller_class_name_without_nesting}"
    end
    ...
    ***REMOVED*** model controller
    ...
    if @model_controller_class_nesting.empty?
      ...
      @model_controller_class_name = "***REMOVED***{@model_controller_class_nesting}::***REMOVED***{@model_controller_class_name_without_nesting}"
    end
  ...
  def manifest
    recorded_session = record do |m|
      ***REMOVED*** Check for class naming collisions.
      m.class_collisions controller_class_path,       "***REMOVED***{controller_class_name}Controller", ***REMOVED*** Sessions Controller
                                                      "***REMOVED***{controller_class_name}Helper"
      m.class_collisions model_controller_class_path, "***REMOVED***{model_controller_class_name}Controller", ***REMOVED*** Model Controller
                                                      "***REMOVED***{model_controller_class_name}Helper"
      m.class_collisions class_path,                  "***REMOVED***{class_name}", "***REMOVED***{class_name}Mailer", "***REMOVED***{class_name}MailerTest", "***REMOVED***{class_name}Observer"
      m.class_collisions [], 'AuthenticatedSystem', 'AuthenticatedTestHelper'

      ***REMOVED*** Controller, helper, views, and test directories.
      m.directory File.join('app/models', class_path)
      m.directory File.join('app/controllers', controller_class_path)
      m.directory File.join('app/controllers', model_controller_class_path)
      m.directory File.join('app/helpers', controller_class_path)
      m.directory File.join('app/views', controller_class_path, controller_file_name)
      m.directory File.join('app/views', class_path, "***REMOVED***{file_name}_mailer") if options[:include_activation]
      ...
      m.template 'model.rb',
                  File.join('app/models',
                            class_path,
                            "***REMOVED***{file_name}.rb")

      if options[:include_activation]
        %w( mailer observer ).each do |model_type|
          m.template "***REMOVED***{model_type}.rb", File.join('app/models',
                                               class_path,
                                               "***REMOVED***{file_name}_***REMOVED***{model_type}.rb")
        end
      end
      ...
      m.template 'controller.rb',
                  File.join('app/controllers',
                            controller_class_path,
                            "***REMOVED***{controller_file_name}_controller.rb")

      m.template 'model_controller.rb',
                  File.join('app/controllers',
                            model_controller_class_path,
                            "***REMOVED***{model_controller_file_name}_controller.rb")
      ...
      if @rspec
        ***REMOVED*** RSpec Specs
        m.template  'spec/controllers/users_controller_spec.rb',
                    File.join('spec/controllers',
                              model_controller_class_path,
                              "***REMOVED***{model_controller_file_name}_controller_spec.rb")
        m.template  'spec/controllers/sessions_controller_spec.rb',
                    File.join('spec/controllers',
                              controller_class_path,
                              "***REMOVED***{controller_file_name}_controller_spec.rb")
        ...
        ***REMOVED*** Cucumber features
        ...
        m.template  'features/step_definitions/user_steps.rb',
         File.join('features/step_definitions/', "***REMOVED***{file_name}_steps.rb")
        ...
        m.template 'test/functional_test.rb',
                    File.join('test/functional',
                              controller_class_path,
                              "***REMOVED***{controller_file_name}_controller_test.rb")
        m.template 'test/model_functional_test.rb',
                    File.join('test/functional',
                              model_controller_class_path,
                              "***REMOVED***{model_controller_file_name}_controller_test.rb")
        m.template 'test/unit_test.rb',
                    File.join('test/unit',
                              class_path,
                              "***REMOVED***{file_name}_test.rb")
        if options[:include_activation]
          m.template 'test/mailer_test.rb', File.join('test/unit', class_path, "***REMOVED***{file_name}_mailer_test.rb")
        end
        m.template 'spec/fixtures/users.yml',
      ...
      m.template 'helper.rb',
                  File.join('app/helpers',
                            controller_class_path,
                            "***REMOVED***{controller_file_name}_helper.rb")

      m.template 'model_helper.rb',
                  File.join('app/helpers',
                            model_controller_class_path,
                            "***REMOVED***{model_controller_file_name}_helper.rb")


      ***REMOVED*** Controller templates
      ...
      m.template '_model_partial.html.erb', File.join('app/views', model_controller_class_path, model_controller_file_name, "_***REMOVED***{file_name}_bar.html.erb")

      if options[:include_activation]
        ***REMOVED*** Mailer templates
        %w( activation signup_notification ).each do |action|
          m.template "***REMOVED***{action}.erb",
                     File.join('app/views', "***REMOVED***{file_name}_mailer", "***REMOVED***{action}.erb")
        end
      end

      unless options[:skip_migration]
        m.migration_template 'migration.rb', 'db/migrate', :assigns => {
          :migration_name => "Create***REMOVED***{class_name.pluralize.gsub(/::/, '')}"
        }, :migration_file_name => "create_***REMOVED***{file_path.gsub(/\//, '_').pluralize}"
      end
      unless options[:skip_routes]
        ***REMOVED*** Note that this fails for nested classes -- you're on your own with setting up the routes.
    ...
    ***REMOVED***
    ***REMOVED*** Post-install notes
    ***REMOVED***
    action = File.basename($0) ***REMOVED*** grok the action from './script/generate' or whatever
    case action
    when "generate"
      ...
      if options[:include_activation]
        ...
        puts "    config.active_record.observers = :***REMOVED***{file_name}_observer"
      end
      if options[:aasm]
      ...
      puts %(    map.signup '/signup', :controller => '***REMOVED***{model_controller_file_name}', :action => 'new')
      puts %(    map.login  '/login',  :controller => '***REMOVED***{controller_file_name}', :action => 'new')
      puts %(    map.logout '/logout', :controller => '***REMOVED***{controller_file_name}', :action => 'destroy')
      if options[:include_activation]
        puts %(    map.activate '/activate/:activation_code', :controller => '***REMOVED***{model_controller_file_name}', :action => 'activate', :activation_code => nil)
      end
      if options[:stateful]
        puts  "  and modify the map.resources :***REMOVED***{model_controller_file_name} line to include these actions:"
        puts  "    map.resources :***REMOVED***{model_controller_file_name}, :member => { :suspend => :put, :unsuspend => :put, :purge => :delete }"
      end
      puts
      puts ("-" * 70)
      puts
      if $rest_auth_site_key_from_generator.blank?
        ...
        puts "We've create a new site key in ***REMOVED***{site_keys_file}.  If you have existing"
        ...
        puts "We've reused the existing site key in ***REMOVED***{site_keys_file}.  As always,"
        puts "keep this file safe but don't post it in public."
      end
      puts
      puts ("-" * 70)
    when "destroy"
      ...
      puts "  ***REMOVED*** config.active_record.observers = :***REMOVED***{file_name}_observer"
      puts
      puts ("-" * 70)
      puts
    else
      puts "Didn't understand the action '***REMOVED***{action}' -- you might have missed the 'after running me' instructions."
    end
  ...
  ***REMOVED***
  ***REMOVED*** !! These must match the corresponding routines in by_password.rb !!
  ***REMOVED***
  ...
  ***REMOVED***
  ***REMOVED*** Try to be idempotent:
  ***REMOVED*** pull in the existing site key if any,
  ***REMOVED*** seed it with reasonable defaults otherwise
  ***REMOVED***
  def load_or_initialize_site_keys
    ...
    when defined? REST_AUTH_SITE_KEY
      if (options[:old_passwords]) && ((! REST_AUTH_SITE_KEY.blank?) || (REST_AUTH_DIGEST_STRETCHES != 1))
        raise "You have a site key, but --old-passwords will overwrite it.  If this is really what you want, move the file ***REMOVED***{site_keys_file} and re-run."
      end
...
protected
  ***REMOVED*** Override with your own usage banner.
  def banner
    "Usage: ***REMOVED***{$0} authenticated ModelName [ControllerName]"
  end
  ...
  def dump_generator_attribute_names
    ...
    generator_attribute_names.each do |attr|
      puts "%-40s %s" % ["***REMOVED***{attr}:", self.send(attr)]  ***REMOVED*** instance_variable_get("@***REMOVED***{attr.to_s}"
    end

  end
end

***REMOVED*** ./script/generate authenticated FoonParent::Foon SporkParent::Spork -p --force --rspec --dump-generator-attrs
***REMOVED*** table_name:                              foon_parent_foons
***REMOVED*** file_name:                               foon
***REMOVED*** class_name:                              FoonParent::Foon
***REMOVED*** controller_name:                         SporkParent::Sporks
***REMOVED*** controller_class_path:                   spork_parent
***REMOVED*** controller_file_path:                    spork_parent/sporks
***REMOVED*** controller_class_nesting:                SporkParent
***REMOVED*** controller_class_nesting_depth:          1
***REMOVED*** controller_class_name:                   SporkParent::Sporks
***REMOVED*** controller_singular_name:                spork
***REMOVED*** controller_plural_name:                  sporks
***REMOVED*** controller_routing_name:                 spork
***REMOVED*** controller_routing_path:                 spork_parent/spork
***REMOVED*** controller_controller_name:              sporks
***REMOVED*** controller_file_name:                    sporks
***REMOVED*** controller_table_name:                   sporks
***REMOVED*** controller_plural_name:                  sporks
***REMOVED*** model_controller_name:                   FoonParent::Foons
***REMOVED*** model_controller_class_path:             foon_parent
***REMOVED*** model_controller_file_path:              foon_parent/foons
***REMOVED*** model_controller_class_nesting:          FoonParent
***REMOVED*** model_controller_class_nesting_depth:    1
***REMOVED*** model_controller_class_name:             FoonParent::Foons
***REMOVED*** model_controller_singular_name:          foons
***REMOVED*** model_controller_plural_name:            foons
***REMOVED*** model_controller_routing_name:           foon_parent_foons
***REMOVED*** model_controller_routing_path:           foon_parent/foons
***REMOVED*** model_controller_controller_name:        foons
***REMOVED*** model_controller_file_name:              foons
***REMOVED*** model_controller_singular_name:          foons
***REMOVED*** model_controller_table_name:             foons
***REMOVED*** model_controller_plural_name:            foons

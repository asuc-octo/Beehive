class <%= model_controller_class_name %>Controller < ApplicationController
  ***REMOVED*** Be sure to include AuthenticationSystem in Application Controller instead
  ...
  <% if options[:stateful] %>
  ***REMOVED*** Protect these actions behind an admin login
  ***REMOVED*** before_filter :admin_required, :only => [:suspend, :unsuspend, :destroy, :purge]
...
<% end -%>
    if success && @<%= file_name %>.errors.empty?
      ...
      ***REMOVED*** protection if visitor resubmits an earlier form using back
      ***REMOVED*** button. Uncomment if you understand the tradeoffs.
      ***REMOVED*** reset session
      self.current_<%= file_name %> = @<%= file_name %> ***REMOVED*** !! now logged in
...
<% end %><% if options[:stateful] %>
  ...
  ***REMOVED*** There's no page here to update or destroy a <%= file_name %>.  If you add those, be
  ***REMOVED*** smart -- make sure you check that the visitor is authorized to do so, that they
  ***REMOVED*** supply their old password along with a new one to update it, etc.

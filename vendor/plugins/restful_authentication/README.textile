...
* "Installation":***REMOVED***INSTALL
* "New Features":***REMOVED***AWESOME
* "After installing":***REMOVED***POST-INSTALL
...
**IMPORTANT FOR RAILS > 2.1 USERS** To avoid a @NameError@ exception ("lighthouse tracker ticket":http://rails_security.lighthouseapp.com/projects/15332-restful_authentication/tickets/2-not-a-valid-constant-name-errors***REMOVED***ticket-2-2), check out the code to have an _underscore_ and not _dash_ in its name:
...
* --old-passwords: Use the older password scheme (see [[***REMOVED***COMPATIBILITY]], above)

module Authentication
  ...
  self.login_regex       = /\A\w[\w\.\-_@]+\z/                     ***REMOVED*** ASCII, strict
  ***REMOVED*** self.login_regex       = /\A[[:alnum:]][[:alnum:]\.\-_@]+\z/     ***REMOVED*** Unicode, strict
  ***REMOVED*** self.login_regex       = /\A[^[:cntrl:]\\<>\/&]*\z/              ***REMOVED*** Unicode, permissive

  self.bad_login_message = "use only letters, numbers, and .-_@ please.".freeze

  self.name_regex        = /\A[^[:cntrl:]\\<>\/&]*\z/              ***REMOVED*** Unicode, permissive
  self.bad_name_message  = "avoid non-printing characters and \\&gt;&lt;&amp;/ please.".freeze

  self.email_name_regex  = '[\w\.%\+\-]+'.freeze
  self.domain_head_regex = '(?:[A-Z0-9\-]+\.)+'.freeze
  self.domain_tld_regex  = '(?:[A-Z]{2}|com|org|net|edu|gov|mil|biz|info|mobi|name|aero|jobs|museum)'.freeze
  self.email_regex       = /\A***REMOVED***{email_name_regex}@***REMOVED***{domain_head_regex}***REMOVED***{domain_tld_regex}\z/i

...
class ActsAsIndexedTest < ActiveSupport::TestCase
  ...
  ***REMOVED*** After a portion of a record has been removed
  ***REMOVED*** the portion removes should no longer be in the index.
  ...
  def test_negative_queries
    ...
    assert_equal [],  Post.find_with_index('-foo') ***REMOVED***Edgecase
  end
  ...
  def test_scoped_negative_queries
    ...
    assert_equal [],  Post.with_query('-foo') ***REMOVED***Edgecase
  end
  ...
  def test_negative_quoted_queries
    ...
    assert_equal [],  Post.find_with_index('-"crane big"',{},{:ids_only => true}) ***REMOVED*** Edgecase
  end

  def test_scoped_negative_quoted_queries
    assert_equal [6],  Post.with_query('crane -"crane ship"').map(&:id)
    assert_equal [],  Post.with_query('-"crane big"') ***REMOVED*** Edgecase
  end
  ...
  ***REMOVED*** When a atom already in a record is duplicated, it removes 
  ***REMOVED*** all records with that same atom from the index.

...
module ActsAsIndexed ***REMOVED***:nodoc:
  class SearchIndex
    ...
    ***REMOVED*** root:: Location of index on filesystem.
    ***REMOVED*** index_depth:: Degree of index partitioning.
    ***REMOVED*** fields:: Fields or instance methods of ActiveRecord model to be indexed.
    ***REMOVED*** min_word_size:: Smallest query term that will be run through search.
    ...
    ***REMOVED*** Adds +record+ to the index.
    ...
    ***REMOVED*** Adds multiple records to the index. Accepts an array of +records+.
    ...
    ***REMOVED*** Removes +record+ from the index.
    def remove_record(record)
      ...
      atoms.each do |a|
        ...
        @records_size -= 1
        ***REMOVED***p "removing ***REMOVED***{record.id} from ***REMOVED***{a}"
      end
    end

    def update_record(record_new, record_old)
      ***REMOVED*** Work out which atoms have modifications.
      ***REMOVED*** Minimises loading and saving of partitions.
      old_atoms = condense_record(record_old)
      new_atoms = condense_record(record_new)

      ***REMOVED*** Remove the old version from the appropriate atoms.
      load_atoms(old_atoms)
      old_atoms.each do |a|
        @atoms[a].remove_record(record_new.id) if @atoms.has_key?(a)
      end

      ***REMOVED*** Add the new version to the appropriate atoms.
      load_atoms(new_atoms)
      ***REMOVED*** TODO: Make a version of this method that takes the
      ***REMOVED*** atomised version of the record.
      add_occurences(new_atoms, record_new.id)
    end

    ***REMOVED*** Saves the current index partitions to the filesystem.
    def save
      prepare
      atoms_sorted = {}
    ...
    ***REMOVED*** Deletes the current model's index from the filesystem.
    ***REMOVED***--
    ***REMOVED*** TODO: Write a public method that will delete all indexes.
    def destroy
      FileUtils.rm_rf(@root)
      true
    end

    ***REMOVED*** Returns an array of IDs for records matching +query+.
    def search(query)
      return [] if query.nil?
    ...
    ***REMOVED*** Returns true if the index root exists on the FS.
    ***REMOVED***--
    ***REMOVED*** TODO: Make a private method called 'root_exists?' which checks for the root directory.
    def exists?
    ...
    ***REMOVED*** Gets the size file from the index.
    ...
    ***REMOVED*** Saves the size to the size file.
    ...
    ***REMOVED*** Returns true if the given atom is present.
    def include_atom?(atom)
      @atoms.has_key?(atom)
    end

    ***REMOVED*** Returns true if all the given atoms are present.
    ...
    ***REMOVED*** Returns true if the given record is present.
    ...
    def add_occurences(condensed_record,record_id)
      condensed_record.each_with_index do |atom, i|
        ...
        ***REMOVED***p "adding ***REMOVED***{record.id} to ***REMOVED***{atom}"
      end
    end
    ...
    ***REMOVED*** Allows compatibility with 1.8.6 which has no ord method.
    ...
    def run_queries(atoms)
      ...
      atoms.uniq.each do |atom|
        ...
        if include_atom?(atom)
          ***REMOVED*** Collect all the weightings for the current atom.
          interim_results = @atoms[atom].weightings(@records_size)
        end
        if results.empty?
          ***REMOVED*** If first time round, set results with initial weightings.
          results = interim_results
        else
          ***REMOVED*** If second time round, add weightings together for records
          ***REMOVED*** matching both atoms. Any matching only one are discarded.
          rr = {}
    ...
    def run_quoted_queries(quoted_atoms)
      ...
      quoted_atoms.each do |quoted_atom|
        interim_results = {}
        ***REMOVED*** Check the index contains all the required atoms.
        ***REMOVED*** match_atom = first_word_atom
        ***REMOVED*** for each of the others
        ***REMOVED***   return atom containing records + positions where current atom is preceded by following atom.
        ***REMOVED*** end
        ***REMOVED*** return records from final atom.
        ...
        ***REMOVED***results += matches.record_ids
    ...
    def load_atoms(atoms)
      ***REMOVED*** Remove duplicate atoms.
      ***REMOVED*** Remove atoms already in index.
      ***REMOVED*** Calculate prefixes.
      ***REMOVED*** Remove duplicate prefixes.
    ...
    def prepare
      ***REMOVED*** Makes the RAILS_ROOT/index directory
      Dir.mkdir(File.join(@root[0,2])) if !File.exists?(File.join(@root[0,2]))
      ***REMOVED*** Makes the RAILS_ROOT/index/ENVIRONMENT directory
      Dir.mkdir(File.join(@root[0,3])) if !File.exists?(File.join(@root[0,3]))
      ***REMOVED*** Makes the RAILS_ROOT/index/ENVIRONMENT/CLASS directory

...
describe Xapit::AbstractIndexer do
  before(:each) do
    XapitMember.xapit { } ***REMOVED*** to ensure methods are included
  ...
  it "should add terms, values and options for facets" do
    Xapit::Config.writable_database ***REMOVED*** force xapit to use a writable database at the beginning
    ...
    @index.facet(:foo)
    @indexer.facet_terms(member).should == ids.map { |id| "F***REMOVED***{id}" }
  ...
  it "should convert time to integer before saving as field term" do
    ...
    @indexer.field_terms(member).should == ["Xcreated_at-***REMOVED***{member.created_at.to_i}"]
  end
  ...
  it "should convert date to time then integer before saving as field term" do
    ...
    @indexer.field_terms(member).should == ["Xcreated_on-***REMOVED***{member.created_on.to_time.to_i}"]
  end

Given /^I configured the database to be saved at "([^\"]*)"$/ do |path|
  Xapit.setup(:database_path => File.dirname(__FILE__) + "/../../***REMOVED***{path}")
end
...
Then /^I should have the following facets$/ do |facets_table|
  ...
  result.should == facets_table.hashes ***REMOVED*** this is somewhat fragile because it depends on order of hash result
end

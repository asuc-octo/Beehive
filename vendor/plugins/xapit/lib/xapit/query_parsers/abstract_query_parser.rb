  ...
  class AbstractQueryParser
    ...
    def initial_query_strings
      if classes.empty?
        ...
        classes.map { |klass| "C***REMOVED***{klass.name}" }
      end
    end
    ...
    def condition_term(name, value)
      if value.kind_of?(Range) && @member_class
        ...
        wildcard_query(value, "X***REMOVED***{name}-")
      else
    ...
    ***REMOVED*** Expands the wildcard in the term (just at the end) and returns a query
    ***REMOVED*** which will match any term that starts with the given term.
    def wildcard_query(term, prefix = "")
      full_term = (prefix + term.downcase).sub(/\*$/, '') ***REMOVED*** remove asterisk at end if it exists

module Xapit
  class AbstractIndexer
    ...
    def update_member(member)
      database.replace_document("Q***REMOVED***{member.class}-***REMOVED***{member.id}", document_for(member))
    end
    
    def document_for(member)
      document = Xapian::Document.new
      document.data = "***REMOVED***{member.class}-***REMOVED***{member.id}"
    ...
    def index_text_attributes(member, document)
      ***REMOVED*** to be overridden by subclass
    end
    ...
    def base_terms(member)
      ["C***REMOVED***{member.class}", "Q***REMOVED***{member.class}-***REMOVED***{member.id}"]
    end
    ...
    def field_terms(member)
      @blueprint.field_attributes.map do |name|
        [member.send(name)].flatten.map do |value|
          ...
          "X***REMOVED***{name}-***REMOVED***{value.to_s.downcase}"
        end
      end.flatten
    end
    ...
    def facet_terms(member)
      @blueprint.facets.map do |facet|
        facet.identifiers_for(member).map { |id| "F***REMOVED***{id}" }
      end.flatten
    end
    
    ***REMOVED*** used primarily by search similar functionality
    def text_terms(member) ***REMOVED*** REFACTORME some duplicaiton with simple indexer
    ...
    ***REMOVED*** TODO remove duplication with sortable_values

...
***REMOVED*** Search for fixtures first
...
class ActiveSupport::TestCase ***REMOVED***:nodoc:
  ...
  def assert_tag_counts(tags, expected_values)
    ***REMOVED*** Map the tag fixture names to real tag names
    ...
    tags.each do |tag|
      ...
      assert_not_nil value, "Expected count for ***REMOVED***{tag.name} was not provided"
      assert_equal value, tag.count, "Expected value of ***REMOVED***{value} for ***REMOVED***{tag.name}, but was ***REMOVED***{tag.count}"
    end
    
    unless expected_values.empty?
      assert false, "The following tag counts were not present: ***REMOVED***{expected_values.inspect}"
    end
  end
...
class Test::Unit::TestCase ***REMOVED***:nodoc:
  self.use_transactional_fixtures = true
  self.use_instantiated_fixtures  = false

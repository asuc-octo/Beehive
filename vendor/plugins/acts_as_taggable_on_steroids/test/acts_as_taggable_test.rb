...
class ActsAsTaggableOnSteroidsTest < ActiveSupport::TestCase
  ...
  def test_reassign_tag_list
    ...
    ***REMOVED*** Only an update of the posts table should be executed, the other two queries are for savepoints
    assert_queries 3 do
  ...
  def test_new_tags
    assert_equivalent ["Very good", "Nature"], posts(:jonathan_sky).tag_list
    posts(:jonathan_sky).update_attributes!(:tag_list => "***REMOVED***{posts(:jonathan_sky).tag_list}, One, Two")
  ...
  def test_change_case_of_tags
    ...
    ***REMOVED*** The new tag list is not uppercase becuase the AR finders are not case-sensitive
    ***REMOVED*** and find the old tags when re-tagging with the uppercase tags.
  ...
  def test_cached_tag_list_not_used
    ***REMOVED*** Load fixture and column information
    posts(:jonathan_sky).taggings(:reload)
    
    assert_queries 1 do
      ***REMOVED*** Tags association will be loaded
  ...
  def test_clearing_cached_tag_list
    ***REMOVED*** Generate the cached tag list
    posts(:jonathan_sky).save!
...
class ActsAsTaggableOnSteroidsTest < Test::Unit::TestCase
  ...
  def test_reassign_tag_list
    ...
    ***REMOVED*** Only an update of the posts table should be executed
    assert_queries 1 do
      posts(:jonathan_rain).update_attributes!(:tag_list => posts(:jonathan_rain).tag_list.to_s)
    end

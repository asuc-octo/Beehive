module ActsAsSolr ***REMOVED***:nodoc:
  ...
  module InstanceMethods
    ...
    ***REMOVED*** Solr id is <class.name>:<id> to be unique across all models
    def solr_id
      "***REMOVED***{self.class.name}:***REMOVED***{record_id(self)}"
    end

    ***REMOVED*** saves to the Solr index
    def solr_save
      return true if indexing_disabled?
      if evaluate_condition(:if, self) 
        logger.debug "solr_save: ***REMOVED***{self.class.name} : ***REMOVED***{record_id(self)}"
    ...
    ***REMOVED*** remove from index
    def solr_destroy
      return true if indexing_disabled?
      logger.debug "solr_destroy: ***REMOVED***{self.class.name} : ***REMOVED***{record_id(self)}"
      solr_delete solr_id
      solr_commit if configuration[:auto_commit]
      true
    end

    ***REMOVED*** convert instance to Solr document
    def to_solr_doc
      logger.debug "to_solr_doc: creating doc for class: ***REMOVED***{self.class.name}, id: ***REMOVED***{record_id(self)}"
      ...
      ***REMOVED*** iterate through the fields and add them to the document,
      configuration[:solr_fields].each do |field_name, options|
        ***REMOVED***field_type = configuration[:facets] && configuration[:facets].include?(field) ? :facet : :text
        ...
        value = self.send("***REMOVED***{field_name}_for_solr")
        value = set_value_if_nil(field_type) if value.to_s == ""
        
        ***REMOVED*** add the field to the document, but only if it's not the id field
        ***REMOVED*** or the type field (from single table inheritance), since these
        ***REMOVED*** fields have already been added above.
        if field_name.to_s != self.class.primary_key and field_name.to_s != "type"
          suffix = get_solr_field_type(field_type)
          ***REMOVED*** This next line ensures that e.g. nil dates are excluded from the 
          ***REMOVED*** document, since they choke Solr. Also ignores e.g. empty strings, 
          ***REMOVED*** but these can't be searched for anyway: 
          ***REMOVED*** http://www.mail-archive.com/solr-dev@lucene.apache.org/msg05423.html
          next if value.nil? || value.to_s.strip.empty?
          [value].flatten.each do |v|
            v = set_value_if_nil(suffix) if value.to_s == ""
            field = Solr::Field.new("***REMOVED***{solr_name}_***REMOVED***{suffix}" => ERB::Util.html_escape(v.to_s))
      ...
      if configuration[:solr_includes].respond_to?(:each)
        configuration[:solr_includes].each do |association, options|
          ...
          when :has_many, :has_and_belongs_to_many
            ...
            unless records.empty?
              ...
              [data].flatten.each do |value|
                field = Solr::Field.new("***REMOVED***{field_name}_***REMOVED***{suffix}" => value)
                field.boost = validate_boost(field_boost)
                doc << field
              end
            end
          when :has_one, :belongs_to
            ...
            unless record.nil?
              doc["***REMOVED***{field_name}_***REMOVED***{suffix}"] = include_value(record, options)
            end
          end
        end
      end
    ...
    def include_value(record, options)
      if options[:using].is_a? Proc
        ...
        record.attributes.inject([]){|k,v| k << "***REMOVED***{v.first}=***REMOVED***{ERB::Util.html_escape(v.last)}"}.join(" ")
      end
    end
      ...
      case condition
        ...
        when TrueClass
          ...
          if condition_block?(condition)
            ...
            raise(
              ArgumentError,
              "The :***REMOVED***{which_condition} option has to be either a symbol, string (to be eval'ed), proc/method, true/false, or " +

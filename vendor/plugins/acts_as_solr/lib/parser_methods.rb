module ActsAsSolr ***REMOVED***:nodoc:
  module ParserMethods
    ...
    ***REMOVED*** Method used by mostly all the ClassMethods when doing a search
    def parse_query(query=nil, options={}, models=nil)
      ...
      raise "Invalid parameters: ***REMOVED***{(options.keys - valid_options).join(',')}" unless (options.keys - valid_options).empty?
      begin
        ...
        ***REMOVED*** first steps on the facet parameter processing
        if options[:facets]
          query_options[:facets] = {}
          query_options[:facets][:limit] = -1  ***REMOVED*** TODO: make this configurable
          ...
          ***REMOVED*** override the :zeros (it's deprecated anyway) if :mincount exists
          query_options[:facets][:mincount] = options[:facets][:mincount] if options[:facets][:mincount]
          query_options[:facets][:fields] = options[:facets][:fields].collect{|k| "***REMOVED***{k}_facet"} if options[:facets][:fields]
          query_options[:filter_queries] = replace_types([*options[:facets][:browse]].collect{|k| "***REMOVED***{k.sub!(/ *: */,"_facet:")}"}) if options[:facets][:browse]
          query_options[:facets][:queries] = replace_types(options[:facets][:query].collect{|k| "***REMOVED***{k.sub!(/ *: */,"_t:")}"}) if options[:facets][:query]
          
          
          if options[:facets][:dates]
            query_options[:date_facets] = {}
            ***REMOVED*** if options[:facets][:dates][:fields] exists then :start, :end, and :gap must be there
            if options[:facets][:dates][:fields]
              [:start, :end, :gap].each { |k| raise "***REMOVED***{k} must be present in faceted date query" unless options[:facets][:dates].include?(k) }
              query_options[:date_facets][:fields] = []
              options[:facets][:dates][:fields].each { |f|
                if f.kind_of? Hash
                  key = f.keys[0]
                  query_options[:date_facets][:fields] << {"***REMOVED***{key}_d" => f[key]}
                  validate_date_facet_other_options(f[key][:other]) if f[key][:other]
                else
                  query_options[:date_facets][:fields] << "***REMOVED***{f}_d"
                end
              }
            end
            ...
            query_options[:date_facets][:filter]  = replace_types([*options[:facets][:dates][:filter]].collect{|k| "***REMOVED***{k.sub!(/ *:(?!\d) */,"_d:")}"}) if options[:facets][:dates][:filter]
        ...
        if models.nil?
          ***REMOVED*** TODO: use a filter query for type, allowing Solr to cache it individually
          models = "AND ***REMOVED***{solr_type_condition}"
        ...
        query = "(***REMOVED***{query.gsub(/ *: */,"_t:")}) ***REMOVED***{models}"
        order = options[:order].split(/\s*,\s*/).collect{|e| e.gsub(/\s+/,'_t ').gsub(/\bscore_t\b/, 'score')  }.join(',') if options[:order] 
        query_options[:query] = replace_types([query])[0] ***REMOVED*** TODO adjust replace_types to work with String or Array  

        if options[:order]
          ***REMOVED*** TODO: set the sort parameter instead of the old ;order. style.
          query_options[:query] << ';' << replace_types([order], false)[0]
        end
        
        ActsAsSolr::Post.execute(Solr::Request::Standard.new(query_options))
      rescue
        raise "There was a problem executing your search: ***REMOVED***{$!} in ***REMOVED***{$!.backtrace.first}"
      end            
    end
    
    def solr_type_condition
      subclasses.inject("(***REMOVED***{solr_configuration[:type_field]}:***REMOVED***{self.name}") do |condition, subclass|
        condition << " OR ***REMOVED***{solr_configuration[:type_field]}:***REMOVED***{subclass.name}"
      end << ')'
    end
    
    ***REMOVED*** Parses the data returned from Solr
    def parse_results(solr_data, options = {})
      ...
      ids = solr_data.hits.collect {|doc| doc["***REMOVED***{solr_configuration[:primary_key_field]}"]}.flatten
    ...
    def find_objects(ids, options, configuration)
      result = if configuration[:lazy] && configuration[:format] != :ids
        ...
        conditions = [ "***REMOVED***{self.table_name}.***REMOVED***{primary_key} in (?)", ids ]
    ...
    ***REMOVED*** Reorders the instances keeping the order returned from Solr
    def reorder(things, ids)
      ordered_things = Array.new(things.size)
      raise "Out of sync! Found ***REMOVED***{ids.size} items in index, but only ***REMOVED***{things.size} were found in database!" unless things.size == ids.size
    ...
    ***REMOVED*** Replaces the field types based on the types (if any) specified
    ***REMOVED*** on the acts_as_solr call
    def replace_types(strings, include_colon=true)
      ...
      if configuration[:solr_fields]
        configuration[:solr_fields].each do |name, options|
          ...
          field = "***REMOVED***{solr_name}_***REMOVED***{solr_type}***REMOVED***{suffix}"
          strings.each_with_index {|s,i| strings[i] = s.gsub(/***REMOVED***{solr_name.to_s}_t***REMOVED***{suffix}/,field) }
        end
      end
    ...
    ***REMOVED*** Adds the score to each one of the instances found
    ...
    def scorable_record?(record, doc)
      doc_id = doc["***REMOVED***{solr_configuration[:primary_key_field]}"]
      if doc_id.nil?
        doc_id = doc["id"]
        "***REMOVED***{record.class.name}:***REMOVED***{record_id(record)}" == doc_id.first.to_s
      else
    ...
    def validate_date_facet_other_options(options)
      ...
      raise "Invalid option***REMOVED***{'s' if bad_options.size > 1} for faceted date's other param: ***REMOVED***{bad_options.join(', ')}. May only be one of :after, :all, :before, :between, :none" if bad_options.size > 0
    end

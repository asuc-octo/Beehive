...
class FacetedSearchTest < Test::Unit::TestCase
  ...
  ***REMOVED*** The tests below are for faceted search, but make sure you setup
  ***REMOVED*** the fields on your model you'd like to index as a facet field:
  ***REMOVED*** 
  ***REMOVED*** class Electronic < ActiveRecord::Base
  ***REMOVED***   acts_as_solr :facets => [:category, :manufacturer]  
  ***REMOVED*** end
  ***REMOVED*** 
  ***REMOVED*** A basic faceted search using just one facet field
  ...
  ***REMOVED*** Making sure the empty result returned what we expected
  ...
  ***REMOVED*** A basic faceted search using multiple facet fields
  ...
  ***REMOVED*** A basic faceted search using facet queries to get counts. 
  ***REMOVED*** Here are the facets search query meaning:
  ***REMOVED*** "price:[* TO 200]" - Price up to 200
  ***REMOVED*** "price:[200 TO 500]" - Price from 200 to 500
  ***REMOVED*** "price:[500 TO *]" - Price higher than 500
  ...
  ***REMOVED*** Faceted search specifying the query and fields
  ...
  ***REMOVED*** Faceted searches with :sort and :zeros options turned on/off
  ...
  ***REMOVED*** Faceted search with 'drill-down' option being passed.
  ***REMOVED*** The :browse option receives the argument in the format:
  ***REMOVED*** "facet_field:term". You can drill-down to as many
  ***REMOVED*** facet fields as you like
  ...
  def test_faceted_search_with_dates_filter
    records = Electronic.find_by_solr "memory", :facets => {:dates => {:filter => ["updated_at:[***REMOVED***{(Date.today - 3.months).strftime("%Y-%m-%dT%H:%M:%SZ")} TO NOW-1MONTH/DAY]"]}}
  ...
  def test_faceted_search_with_dates_filter_and_facets
    ***REMOVED*** this is a very contrived example but gives us data to validate
    records = Electronic.find_by_solr "memory", :facets => {:dates => {:filter => ["updated_at:[***REMOVED***{(Date.today - 3.months).strftime("%Y-%m-%dT%H:%M:%SZ")} TO NOW-1MONTH/DAY]"],

  ...
  ***REMOVED*** Make it a foreign key to Department.
  ...
  def self.up
    ...
    depts.each do |f_id, dept_name|
      Faculty.find(f_id).update_attribute(:department, Department.find_or_create_by_name(dept_name)) || raise("Unable to reset department for faculty ***REMOVED******REMOVED***{f_id}")
    end
  end

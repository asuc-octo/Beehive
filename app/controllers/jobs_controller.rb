class JobsController < ApplicationController
  ...
  ***REMOVED*** Ensures that only the user who created a job -- and no other users -- can edit it 
  ...
  def list
	...
	if(query && !query.empty? && (query != @search_query))
		***REMOVED***@jobs = Job.find_by_solr_by_relevance(query).select { |c| c.active == true } ***REMOVED*** How to filter these results pre-query through solr?  Should actually be filtered through solr, not here.
        @jobs = find_jobs(query)
		***REMOVED***@jobs = @jobs.sort {|a,b| a.created_at <=> b.created_at}		
	else
  ...
  ***REMOVED*** POST /jobs.xml
  def create
	...
	***REMOVED*** Handles the text_field_with_auto_complete for desired proglangs.

  ...
  ***REMOVED*** Ensures that only logged-in users can create, edit, or delete jobs
  ...
  def list
	...
	if(query && !query.empty? && (query != @search_query))
		@jobs = Job.find_by_solr(query).results.select { |c| c.active == true } ***REMOVED*** How to filter these results pre-query through solr?  Should actually be filtered through solr, not here.

class JobsController < ApplicationController
  ...
  ***REMOVED*** Ensures that only logged-in users can create, edit, or delete jobs
  ...
  def list
	...
	if(query && !query.empty? && (query != @search_query))
		@jobs = Job.find_by_solr(query).results.select { |c| c.active == true }.sort {|a,b| a.created_at <=> b.created_at} ***REMOVED*** How to filter these results pre-query through solr?  Should actually be filtered through solr, not here.
  ...
  ***REMOVED*** POST /jobs.xml
  def create
	...
	***REMOVED***params[:job][:category_id] = Category.find_or_create_by_name(params[:category][:name].id)
	params[:job][:category_names] = params[:category][:name]
		***REMOVED*** = Category.find_or_create_by_name(params[:category][:name])

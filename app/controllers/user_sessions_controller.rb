...
***REMOVED***before_filter :goto_home_unless_logged_in, :except => :destroy
  ...
  def new
    ...
    ***REMOVED*** When using OmniAuth, different fields in the Users table can be used for
    ***REMOVED*** auth. Based on the provider, we the change auth_field to look at the
    ***REMOVED*** right Users column and change auth_value to match that provider's auth
    ***REMOVED*** details. Add these on to /config/initializers/omniauth.rb.
    ...
    ***REMOVED*** This is used (temporarily) for the new/create routes for users. Right
    ***REMOVED*** now, those routes are not used in the logic of the app but are still
    ***REMOVED*** available. User creation is done in first_login.

    ***REMOVED*** cas_user should be removed entirely in a full transition away from CAS to
    ***REMOVED*** generic OmniAuth.
    ...
    if login_user!(user)
      ...
      ***REMOVED*** user's first login; redirect already done for us
      return
    else
